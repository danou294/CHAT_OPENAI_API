...
# Assure-toi d'importer la vue StripeCheckoutView
from payments.views import StripeCheckoutView  # Modifie 'payments' selon le nom de ton application
...
urlpatterns = [
    ...
    path('sessions/<int:session_id>/messages/add/', add_message, name='add_message'),  # Ajouter un message à une session
    path('messages/<int:message_id>/delete/', delete_message, name='delete_message'),  # Supprimer un message
    path('sessions/<int:session_id>/messages/', get_messages, name='get_messages'),  # Obtenir les messages d'une session
